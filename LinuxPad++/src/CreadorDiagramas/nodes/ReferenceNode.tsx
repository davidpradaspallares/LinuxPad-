...
export default function ReferenceNode({ id, data: rawData, selected }: NodeProps) {
  ...
  const fill = data.color ?? '#64748b';
  const stroke = selected ? '#60a5fa' : 'rgba(255,255,255,0.2)';
  // Right-pointing triangle
  const pts = `2,2 ${W - 2},${H / 2} 2,${H - 2}`;
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1, padding: '4px 16px 4px 6px' }}>
        {editing ? (
          <input
            ref={inputRef}
            value={draft}

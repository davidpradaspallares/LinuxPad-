...
export default function PreparationNode({ id, data: rawData, selected }: NodeProps) {
  ...
  const fill = data.color ?? '#0369a1';
  const stroke = selected ? '#60a5fa' : 'rgba(255,255,255,0.2)';
  const pts = `${SKEW},1 ${W - SKEW},1 ${W - 1},${H / 2} ${W - SKEW},${H - 1} ${SKEW},${H - 1} 1,${H / 2}`;
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1, padding: `4px ${SKEW + 6}px` }}>
        {editing ? (
          <input
            ref={inputRef}
            value={draft}

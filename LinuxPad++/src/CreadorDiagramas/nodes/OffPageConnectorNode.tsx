...
export default function OffPageConnectorNode({ id, data: rawData, selected }: NodeProps) {
  ...
  const fill = data.color ?? '#1e40af';
  const stroke = selected ? '#60a5fa' : 'rgba(255,255,255,0.2)';
  // Pentagon: flat top, sides slope inward, pointed bottom
  const midY = Math.round(H * 0.62);
  const pts = `2,2 ${W - 2},2 ${W - 2},${midY} ${W / 2},${H - 2} 2,${midY}`;

  return (
    <div
      onDoubleClick={startEdit}
      style={{ width: '100%', height: '100%', minWidth: 44, minHeight: 44, position: 'relative', cursor: 'default', userSelect: 'none' }}
    >
      <NodeResizer minWidth={44} minHeight={44} isVisible={selected} color="#60a5fa" />
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1, padding: `4px 6px ${H - midY + 6}px` }}>
        {editing ? (
          <input
            ref={inputRef}
            value={draft}

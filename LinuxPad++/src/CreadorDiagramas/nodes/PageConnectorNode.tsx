...
export default function PageConnectorNode({ id, data: rawData, selected }: NodeProps) {
  ...
  return (
    ...
    >
      ...
      <svg width="100%" height="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" style={{ position: 'absolute', inset: 0, overflow: 'visible' }}>
        <circle cx={W / 2} cy={H / 2} r={W / 2 - 2} fill={fill} stroke={stroke} strokeWidth={2} />
      </svg>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1, padding: '4px' }}>
        {editing ? (
          <input
            ...
            style={{ background: 'transparent', border: 'none', outline: 'none', color: '#fff', fontWeight: 600, fontSize: 11, textAlign: 'center', width: '100%' }}
            onClick={e => e.stopPropagation()}
          />
        ) : (
          <span style={{ color: '#fff', fontWeight: 600, fontSize: 11, textAlign: 'center', wordBreak: 'break-word' }}>{data.label}</span>
        )}
      </div>
